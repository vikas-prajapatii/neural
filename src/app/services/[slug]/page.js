...
export default function ServiceSlugPage({ params }) {
  ...
  return (
    <div className="relative min-h-screen bg-[#02040c] text-neutral-200 pt-20 font-sans overflow-hidden">
      ...
      {/* Hero Header Section */}
      <div className="relative w-full h-[55vh] min-h-[400px] flex items-center justify-center overflow-hidden border-b border-neutral-900/50 bg-[#02040c]">
        {/* Ambient background glow */}
        <div className="absolute inset-0 z-0">
          <div className="absolute top-[20%] left-[50%] -translate-x-1/2 w-[60%] h-[60%] bg-cyan-950/15 rounded-full blur-[120px] pointer-events-none"></div>
          <div className="absolute inset-0 bg-gradient-to-t from-[#02040c] via-transparent to-[#02040c] z-10" />
        </div>
      ...
      {/* 351 Studio Inspired Vertical Showcase Section */}
      {service.showcaseVideos && (
        <div className="max-w-7xl mx-auto px-6 pt-16 pb-8 border-b border-neutral-900/60 relative z-10">
          <div className="space-y-12">
            <div className="flex items-center space-x-2">
              <div className="h-[2px] w-6 bg-cyan-500" />
              <h2 className="text-xs font-bold text-neutral-400 uppercase tracking-widest">
                Watch Our Short-Form Video AD Production Examples
              </h2>
            </div>
            
            <div className="flex flex-wrap justify-center gap-10 max-w-4xl mx-auto">
              {service.showcaseVideos.map((video, idx) => (
                <div key={idx} className="w-full max-w-[280px] relative aspect-[9/16] rounded-[2.5rem] border-8 border-neutral-800/80 bg-neutral-950 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8),0_0_30px_rgba(6,182,212,0.15)] group hover:shadow-[0_25px_60px_rgba(0,0,0,0.9),0_0_40px_rgba(6,182,212,0.3)] transition-all duration-500 hover:scale-[1.02]">
                  {/* Phone Notch/Speaker Detail */}
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-28 h-5 bg-neutral-800/80 rounded-b-2xl z-30 flex items-center justify-center">
                  ...
                  {/* Title overlay */}
                  <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-6 z-20 pt-16 pointer-events-none transition-opacity duration-300 opacity-100 group-hover:opacity-0">
                    <h3 className="text-xs font-bold text-white tracking-wide uppercase">{video.title}</h3>
                    <p className="text-[9px] text-cyan-400 mt-1 font-mono tracking-wider">9:16 HD LOOP</p>
                  </div>
                </div>
              ))}
            </div>
